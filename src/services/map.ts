...
<head>
    ...
    body{
        ...
        .current-marker {
            ...
            background: rgba(41, 109, 243, 0.1);
            z-index: 9;
        }
        .current-marker .icon {
            width: 12px;
            ...
            box-shadow: 0px 0px 7px 0px rgba(0,79,236,1);
            background: #004fec;
        }
    </style>
...
<body style="margin: 0; padding: 0;">
    ...
    let currentPlaceData = []; // 현재 표시할 장소 데이터를 저장할 변수
    let map
     ...
     * 지도 이동 함수
     */
    function moveMap({ lng, lat, level }) {
        map.setLevel(level)
    ...
    function hideOverlays() {
      if(overlays && overlays.length > 0) {
          overlays.forEach(m => m.setVisible(false))
      }
    }
    ...
    function handleOverlayClick(params) {
        const { lng, lat, level, category, id, type, types } = params;
     ...
     * @param {string} id - 장소id
     * @param {string} category - 카테고리 이넘
     * @param {string} type - tourist | restaurant
     * @param {number} lat - 위도
     * @param {number} lng - 경도
     */
    ...
    if (level <= 3) {
        ...
        // 줌 레벨이 낮을 때는 클러스터링 수행
        // 줌 레벨에 따른 그리드 셀 크기 계산
        const cellSize = getCellSize(level)

        // 클러스터링 수행
        const clusteredData = clusterMarkers(data, cellSize)

        // 클러스터링된 데이터를 이용하여 오버레이 생성
        clusteredData.forEach(cluster => {
        const items = cluster.list
        const avgLat = cluster.avg.latitude
        const avgLng = cluster.avg.longitude

        let overlay
        if (items.length === 1) {
            // 단일 오버레이
            overlay = createSingleOverlay(items[0], level)
        } else {
            overlay = createClusteredOverlay(items, level, avgLat, avgLng)
        }
        overlays.push(overlay)
        overlay.setMap(map)
        })
     ...
     * @param {number} avgLng
     */
    function createClusteredOverlay(items, level, avgLat, avgLng) {
        let keys = '';
        let categories = '';
        let types = '';
        ...
        if (uniqueCategories.size === 1) {
            ...
            categoryArray.forEach(function(category, index) {
                ...
                imagesHtml += '<img class="icon" src="' + imageUrl + '" style="left:' + (index * 12) + 'px; z-index:' + (categoryArray.length - index) + ';" />';
            });
        }

        const countHtml = '<div class="count" style="margin-left:' + (((categoryArray.length - 1) * 15) + 26) + 'px;">+' + items.length + '</div>';

        const content = '<div class="group-overlay" onclick="handleOverlayClick({ lng: ' + avgLng + ', lat: ' + avgLat + ', level: ' + level + ', category: \\\'' + categories + '\\\', id: \\\'' + keys + '\\\', types: \\\'' + types + '\\\' })">' +
            '<div class="icons">' + imagesHtml + countHtml + '</div>' +
            '<div class="arrow"></div>' +
            '</div>';
     ...
     */
    function getGridCoordinates(lat, lng, cellSize) {
    ...
    /**
     * 마커들을 그리드 셀에 따라 클러스터링합니다.
     * @param {Content[]} markers - 마커 배열
     * @param {number} cellSize - 그리드 셀 크기
     * @returns {Object[]} 클러스터링된 결과
     */
    function clusterMarkers(markers, cellSize) {
        const groups = {};

        markers.forEach(marker => {
            const { x, y } = getGridCoordinates(marker.lat, marker.lng, cellSize);
            const key = x + ',' + y;

            if (!groups[key]) {
                groups[key] = { centerLatitude: 0, centerLongitude: 0, count: 0, items: [] };
            }

            groups[key].centerLatitude += marker.lat;
            groups[key].centerLongitude += marker.lng;
            groups[key].count += 1;
            groups[key].items.push(marker);
        });

        return Object.values(groups).map(group => ({
            avg: {
                latitude: group.centerLatitude / group.count,
                longitude: group.centerLongitude / group.count,
            },
            list: group.items,
        }));
    }
    ...
    /**
     * 줌 레벨에 따른 그리드 셀 크기를 반환합니다.
     * @param {number} level - 지도 줌 레벨
     * @returns {number} 그리드 셀 크기
     */
    ...
    /**
     * @typedef {Record} Place
     * @param {string} id - 기록id
     * @param {string} category - 'RECORD'
    ...
    function settingImageOverlays(response) {
        ...
        if (level <= 4) {
            ...
            clusteredData.forEach(cluster => {
                ...
                if (items.length === 1) {
                    // 단일 오버레이
                    overlay = createSingleImageOverlay(items[0], level);
                } else {
                    overlay = createClusteredImageOverlay(items, level, avgLat, avgLng);
                }
                overlays.push(overlay);
                overlay.setMap(map);
            });
    ...
    function createClusteredImageOverlay(items, level) {
        ...
        const countHtml =
            '<div class="count" style="margin-left:' +
            ((imageCount - 1) * 15 + 35) +
            'px;">+' +
            items.length +
            '</div>'
        ...
        const content =
            '<div class="group-overlay" onClick="handleOverlayClick({ lng: ' +
            avgLng +
            ', lat: ' +
            avgLat +
            ', level: ' +
        ...
        const content = '<div class="current-marker"><span class ="current-marker icon"></span></div>';
        const level = map.getLevel();
